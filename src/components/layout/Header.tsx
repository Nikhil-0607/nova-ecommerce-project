...
export default function Header(){
  const {cartItems,wishlist}=useStore(); const [open,setOpen]=useState(false)
  ...
  return <>
    ...
    <header className="header"><div className="container header-inner">
      ...
      <div className="actions"><Link to="/account">Profile</Link><Link to="/wishlist">♡ <b>{wishlist.length}</b></Link><Link to="/cart">Bag <b>{cartQuantity}</b></Link></div>
    </div></header>
  </>
}
